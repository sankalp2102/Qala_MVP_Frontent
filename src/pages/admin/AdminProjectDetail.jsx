...
function BriefDisplay({ brief }) {
  const rows = [
    ...
    ['Target landing',  brief.target_landing_price_usd ? `$${parseFloat(brief.target_landing_price_usd).toLocaleString()}${brief.target_landing_currency ? ` (${brief.target_landing_currency} ${brief.target_landing_price_local})` : ''}` : '—'],
...
function ProposalReviewPanel({ proposal, projectId, brief, onRefresh }) {
  ...
  const [lineItems, setLineItems] = useState(proposal.line_items || []);
  const [boxes,     setBoxes]     = useState(proposal.boxes || []);
  const [orderType, setOrderType] = useState(proposal.order_type || 'production');
  const [domain,    setDomain]    = useState(proposal.product_domain || 'apparel');
  ...
  const materials = isJew ? JEW_MATERIALS : isAcc ? ACC_MATERIALS : MATERIALS;
  ...
  const saveChanges = async () => {
    ...
    try {
      await projectsAPI.adminUpdateProposal(projectId, proposal.id, {
        order_type: orderType, product_domain: domain, shipping_method: shipping,
      ...
      <div style={{ background:'var(--surface2)',border:'1px solid var(--border)',borderRadius:10,padding:'14px 18px',marginBottom:16 }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))',gap:12 }}>
          {[
            ...
            ['Category',       brief?.product_category || '—'],
            ['Sample qty',     brief?.sample_quantity ? `${brief.sample_quantity} set` : '—'],
            ['Bulk qty',       brief?.bulk_quantity   ? `${brief.bulk_quantity} sets` : '—'],
            ['Target landing', targetUSD ? `${brief.target_landing_currency||'USD'} ${brief.target_landing_price_local||''} ≈ ${fmtUSD(targetUSD)}` : '—'],
            ...
            <div>
              ...
              <div style={{ background:'var(--surface)',border:'1px solid var(--border)',borderRadius:10,padding:'16px 18px',marginBottom:14 }}>
                <div style={{ display:'flex',gap:24,flexWrap:'wrap' }}>
                  <div>
                    <div style={{fontSize:10,fontWeight:700,color:'var(--text4)',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:8}}>Order type</div>
                    <div style={{display:'flex',gap:6}}>
                      {[['designing','Designing'],['sampling','Sampling'],['production','Production']].map(([v,l]) => (
                        <button key={v} onClick={() => setOrderType(v)} style={togBtn(v,orderType)}>{l}</button>
                      ))}
                    </div>
                  </div>
                  <div>
                    <div style={{fontSize:10,fontWeight:700,color:'var(--text4)',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:8}}>Product domain</div>
                    <div style={{display:'flex',gap:6}}>
                      {[['apparel','Apparel'],['home_furnishings','Home Furnishings'],['jewellery','Jewellery'],['accessories','Accessories']].map(([v,l]) => (
                        <button key={v} onClick={() => { setDomain(v); setLineItems([]); }} style={togBtn(v,domain)}>{l}</button>
                      ))}
                    </div>
                  </div>
                  <div>
              ...
              <div style={{ background:'var(--surface)',border:'1px solid var(--border)',borderRadius:10,padding:'16px 18px',marginBottom:14 }}>
                ...
                {lineItems.length === 0 ? (
                  <div style={{fontSize:12,color:'var(--text4)',fontStyle:'italic',padding:'16px 0',textAlign:'center'}}>
                    No items submitted yet. The studio may not have added items, or the data may still be syncing.
                  </div>
                ) : (
                    ...
                    <table style={{width:'100%',borderCollapse:'collapse',fontSize:11,minWidth:560}}>
                      <thead>
                        <tr style={{background:'var(--surface2)'}}>
                          {['Item','Category',...(isApparel?['Gender','Technique']:['Material']),'Wt/pc','Qty','Cost/pc (₹)','GST',...(orderType==='sampling'?['Decl. val ($)']:[]),''].map(h => (
                            <th key={h} style={{padding:'6px 5px',textAlign:'left',fontWeight:600,color:'var(--text3)',fontSize:10,whiteSpace:'nowrap'}}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {lineItems.map((it,i) => (
                          <tr key={it._id||i} style={{borderBottom:'1px solid var(--border)'}}>
                            <td style={{padding:'4px 4px'}}><input value={it.name||''} onChange={e => setLineItems(li=>li.map((x,xi)=>xi===i?{...x,name:e.target.value}:x))} style={inp(90)} /></td>
                            <td style={{padding:'4px 4px'}}><select value={it.category||cats[0]} onChange={e => setLineItems(li=>li.map((x,xi)=>xi===i?{...x,category:e.target.value}:x))} style={inp(110)}>{cats.map(c=><option key={c}>{c}</option>)}</select></td>
                            {isApparel && <td style={{padding:'4px 4px'}}><select value={it.gender||'Women'} onChange={e => setLineItems(li=>li.map((x,xi)=>xi===i?{...x,gender:e.target.value}:x))} style={inp(66)}>{GENDERS.map(g=><option key={g}>{g}</option>)}</select></td>}
                            {isApparel && <td style={{padding:'4px 4px'}}><select value={it.technique||'Woven'} onChange={e => setLineItems(li=>li.map((x,xi)=>xi===i?{...x,technique:e.target.value}:x))} style={inp(100)}>{["Woven","Knitted / Crocheted"].map(t=><option key={t}>{t}</option>)}</select></td>}
                            {!isApparel && <td style={{padding:'4px 4px'}}><select value={it.material||materials[0]} onChange={e => setLineItems(li=>li.map((x,xi)=>xi===i?{...x,material:e.target.value}:x))} style={inp(100)}>{materials.map(m=><option key={m}>{m}</option>)}</select></td>}
                            <td style={{padding:'4px 4px'}}><input type="number" value={it.weight_per_pc||''} onChange={e => setLineItems(li=>li.map((x,xi)=>xi===i?{...x,weight_per_pc:e.target.value}:x))} style={inp(56)} /></td>
                            <td style={{padding:'4px 4px'}}><input type="number" value={it.qty||''} onChange={e => setLineItems(li=>li.map((x,xi)=>xi===i?{...x,qty:e.target.value}:x))} style={inp(46)} /></td>
                            <td style={{padding:'4px 4px'}}><input type="number" value={it.cost_per_pc_inr||''} onChange={e => setLineItems(li=>li.map((x,xi)=>xi===i?{...x,cost_per_pc_inr:e.target.value}:x))} style={inp(76)} /></td>
                            <td style={{padding:'4px 4px'}}><select value={it.gst_rate||'0.12'} onChange={e => setLineItems(li=>li.map((x,xi)=>xi===i?{...x,gst_rate:e.target.value}:x))} style={inp(54)}>{GST_OPTIONS.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></td>
                            {orderType === 'sampling' && <td style={{padding:'4px 4px'}}><input type="number" value={it.declared_value_usd||''} onChange={e => setLineItems(li=>li.map((x,xi)=>xi===i?{...x,declared_value_usd:e.target.value}:x))} style={inp(72)} /></td>}
                            <td style={{padding:'4px 4px'}}><button onClick={() => setLineItems(li=>li.filter((_,xi)=>xi!==i))} style={{background:'none',border:'none',color:'var(--red)',cursor:'pointer',fontSize:16}}>×</button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <button onClick={() => setLineItems(li => [...li, {_id:Date.now(),name:'',category:cats[0],gender:'Women',technique:'Woven',material:materials[0],weight_per_pc:'',qty:'',cost_per_pc_inr:'',gst_rate:'0.12',declared_value_usd:''}])}
                  style={{marginTop:10,fontSize:11,color:'var(--gold)',background:'var(--gold-dim)',border:'1px solid rgba(200,165,90,0.2)',borderRadius:5,padding:'5px 12px',cursor:'pointer',fontFamily:'var(--font-body)'}}>
                  + Add item
                </button>
              </div>
