...
const S = {
  ...
  navChip: { background: 'rgba(255,255,255,0.15)', color: '#fff', borderRadius: 100, padding: '3px 10px', fontSize: 11, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase' },
  ...
  input: { width: '100%', fontSize: 14, color: 'var(--text)', background: 'var(--surface)', border: '1px solid var(--border2)', borderRadius: 8, padding: '10px 14px', outline: 'none', boxSizing: 'border-box' },

  buyerDropdown: { position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, zIndex: 50, background: '#fff', border: '1px solid var(--border2)', borderRadius: 10, boxShadow: '0 8px 24px rgba(0,0,0,0.10)', overflow: 'hidden' },
  ...
  buyerSelectedCard: { background: 'var(--admin-dim)', border: '1px solid rgba(91,75,138,0.20)', borderRadius: 10, padding: '14px 16px', marginTop: 8, display: 'flex', alignItems: 'center', gap: 14 },
  ...
  tag: (selected) => ({ padding: '7px 14px', borderRadius: 100, border: `1px solid ${selected ? 'var(--gold-d)' : 'var(--border2)'}`, fontSize: 13, fontWeight: selected ? 500 : 400, color: selected ? '#fff' : 'var(--text2)', cursor: 'pointer', background: selected ? 'var(--gold)' : 'var(--surface)', userSelect: 'none' }),
  tagAddWrap: { display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 },
  tagAddInput: { fontSize: 13, padding: '6px 12px', border: '1px dashed var(--border2)', borderRadius: 100, outline: 'none', background: 'var(--bg)', color: 'var(--text)', width: 160 },
  tagAddBtn: { padding: '6px 12px', borderRadius: 100, border: '1px solid var(--border2)', background: 'var(--surface)', fontSize: 12, fontWeight: 600, color: 'var(--admin)', cursor: 'pointer', whiteSpace: 'nowrap' },

  priceRow: { display: 'flex', gap: 10 },

  uploadZone: { border: '1.5px dashed var(--border2)', borderRadius: 10, padding: '28px 20px', textAlign: 'center', cursor: 'pointer', background: 'var(--surface)' },
  uploadTitle: { fontSize: 14, fontWeight: 500, color: 'var(--text2)', marginBottom: 4 },
  uploadSub: { fontSize: 12, color: 'var(--text4)' },
  uploadFileItem: { display: 'flex', alignItems: 'center', gap: 10, background: '#fff', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 14px' },
  ...
  qalaCommentBox: { background: 'var(--admin-dim)', border: '1px solid rgba(91,75,138,0.20)', borderRadius: 10, padding: 16 },
  ...
  btn: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 14, fontWeight: 500, padding: '11px 22px', borderRadius: 8, border: 'none', cursor: 'pointer' },
  ...
  briefValueMono: { fontFamily: 'monospace', fontSize: 12, letterSpacing: '.04em', background: 'rgba(255,255,255,0.12)', padding: '2px 8px', borderRadius: 4, display: 'inline-block' },
  ...
  countChip: { background: 'var(--surface2)', border: '1px solid var(--border2)', borderRadius: 100, padding: '3px 10px', fontSize: 12, fontWeight: 600, color: 'var(--text2)' },
  searchBar: { display: 'flex', alignItems: 'center', gap: 10, background: '#fff', border: '1px solid var(--border2)', borderRadius: 10, padding: '10px 14px', marginBottom: 16 },
  ...
  filterChip: (active) => ({ padding: '6px 14px', borderRadius: 100, border: `1px solid ${active ? 'var(--admin)' : 'var(--border2)'}`, background: active ? 'var(--admin)' : '#fff', fontSize: 12, fontWeight: 500, color: active ? '#fff' : 'var(--text2)', cursor: 'pointer', userSelect: 'none' }),

  studioGrid: { display: 'flex', flexDirection: 'column', gap: 12 },
  studioCard: (assigned) => ({ border: `1px solid ${assigned ? 'var(--gold)' : 'var(--border)'}`, borderRadius: 12, padding: '18px 20px', display: 'flex', alignItems: 'flex-start', gap: 16, background: assigned ? 'var(--gold-dim)' : '#fff' }),
  studioAvatar: { width: 48, height: 48, borderRadius: 10, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 700, background: 'var(--admin-dim)', color: 'var(--admin)' },
  ...
  studioTag: { padding: '3px 10px', borderRadius: 100, border: '1px solid var(--border)', fontSize: 11, color: 'var(--text2)', background: 'var(--surface)' },
  studioMeta: { display: 'flex', gap: 16, flexWrap: 'wrap' },
  studioStat: { fontSize: 11, color: 'var(--text3)' },
  assignBtn: (assigned) => ({ flexShrink: 0, padding: '8px 18px', borderRadius: 8, border: `1px solid ${assigned ? 'var(--gold-d)' : 'var(--border2)'}`, background: assigned ? 'var(--gold)' : 'var(--surface)', fontSize: 13, fontWeight: 500, color: assigned ? '#fff' : 'var(--text2)', cursor: 'pointer', whiteSpace: 'nowrap', marginTop: 2 }),
  ...
  assignedItem: { display: 'flex', alignItems: 'center', gap: 12, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px' },
  aiAvatar: { width: 36, height: 36, borderRadius: 8, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 700, background: 'var(--admin-dim)', color: 'var(--admin)' },
  ...
  confirmPill: { display: 'flex', alignItems: 'center', gap: 10, background: 'var(--gold-dim)', border: '1px solid var(--gold-l)', borderRadius: 8, padding: '10px 12px', textAlign: 'left' },
  ...
  modal: { background: '#fff', borderRadius: 16, width: '100%', maxWidth: 640, maxHeight: '88vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 60px rgba(0,0,0,0.20)' },
  modalHeader: { padding: '22px 24px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', gap: 16 },
  modalAvatar: { width: 56, height: 56, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 700, flexShrink: 0, background: 'var(--admin-dim)', color: 'var(--admin)' },
  ...
  modalStat: { background: 'var(--surface)', borderRadius: 10, padding: '12px 14px' },
  ...
  modalTag: { padding: '4px 12px', borderRadius: 100, border: '1px solid var(--border2)', fontSize: 12, color: 'var(--text2)', background: 'var(--surface)' },
  modalFooter: { padding: '16px 24px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10, alignItems: 'center' },

  matchBadge: (m) => ({ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, padding: '3px 9px', borderRadius: 100, flexShrink: 0, background: m.bg, color: m.color }),
  matchBarWrap: { display: 'flex', alignItems: 'center', gap: 6, marginTop: 6 },
  matchBar: { height: 4, borderRadius: 2, background: 'var(--surface2)', overflow: 'hidden', flex: 1 },
  matchBarFill: (pct) => ({ height: '100%', borderRadius: 2, background: 'var(--gold)', width: `${pct}%` }),
};
    ...
    <div>
      ...
      {step === 1 && (
        <div style={S.page}>
          ...
          {error && <div style={{ background: 'var(--red-dim)', color: 'var(--red)', padding: '10px 14px', borderRadius: 8, fontSize: 13, marginBottom: 20 }}>{error}</div>}
