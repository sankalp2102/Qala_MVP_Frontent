...
/* ══════════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════ */
const STUDIO_V3_CSS = `
.studio-v3 { font-family:'DM Sans',system-ui,sans-serif; background:var(--bg); color:var(--text-heading); font-size:14px; line-height:1.6; min-height:100vh; }
...
/* ── TOP NAV ── */
.studio-v3 .top-nav { position:fixed; top:0; left:0; right:0; height:50px; background:rgba(255,255,255,0.96); backdrop-filter:blur(10px); display:flex; align-items:center; justify-content:space-between; padding:0 28px; z-index:200; border-bottom:1px solid var(--surface3); }
.studio-v3 .nav-left { display:flex; align-items:center; gap:16px; min-width:0; }
.studio-v3 .nav-back { display:flex; align-items:center; gap:6px; color:var(--sage); background:none; border:none; padding:0; font-family:'DM Sans',sans-serif; font-size:13px; cursor:pointer; flex-shrink:0; }
.studio-v3 .nav-back:hover { color:var(--text-heading); }
.studio-v3 .nav-studio-name { font-size:14px; font-weight:500; color:var(--text-heading); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
...
/* ── HERO ── */
.studio-v3 .hero { margin-top:50px; height:72vh; min-height:440px; position:relative; overflow:hidden; background:var(--surface3); }
.studio-v3 .hero-bg { position:absolute; inset:0; width:100%; height:100%; object-fit:cover; display:block; }
.studio-v3 .hero-bg-empty { position:absolute; inset:0; background:linear-gradient(160deg,var(--text) 0%,var(--text-heading) 35%,#4a3428 65%,#6a4e3c 100%); display:flex; align-items:center; justify-content:center; font-size:72px; opacity:0.25; }
...
.studio-v3 .sub-label { font-size:10px; letter-spacing:0.15em; text-transform:uppercase; color:var(--text-muted); margin:22px 0 10px; }

/* ── BADGES ── */
.studio-v3 .badge { display:inline-flex; align-items:center; border-radius: var(--r-20); font-size:10px; font-weight:500; letter-spacing:0.06em; padding:3px 9px; text-transform:uppercase; line-height:1; white-space:nowrap; }
.studio-v3 .badge-skilled { background:var(--surface2); color:#5C845C; }
.studio-v3 .badge-expert  { background:#A8D4A8; color:#1A5C1A; }
.studio-v3 .badge-master  { background:var(--green-d); color:#fff; }

/* ── PORTFOLIO ── */
.studio-v3 .portfolio-brief-context { display:flex; align-items:center; gap:8px; margin-bottom:14px; flex-wrap:wrap; }
.studio-v3 .portfolio-brief-label { font-size:10px; letter-spacing:0.12em; text-transform:uppercase; color:var(--text-muted); white-space:nowrap; }
.studio-v3 .portfolio-brief-pill { font-size:11px; color:var(--green-d); background:var(--surface2); border:1px solid #BCCFB8; border-radius: var(--r-20); padding:2px 9px; }
.studio-v3 .portfolio-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:6px; }
.studio-v3 .portfolio-item { position:relative; aspect-ratio:3/5; overflow:hidden; border-radius: var(--r-4); cursor:pointer; background:var(--surface3); }
...
.studio-v3 .portfolio-hover-tag { font-size:10px; color:rgba(255,255,255,0.8); background:rgba(255,255,255,0.15); padding:2px 7px; border-radius: var(--r-20); backdrop-filter:blur(4px); }
/* Product details shown on hover over the FULL image inside the lightbox. */
.studio-v3-lb-hover { position:absolute; inset:0; background:linear-gradient(to top,rgba(10,6,4,0.82) 0%,transparent 45%); opacity:0; transition:opacity 0.2s; display:flex; flex-direction:column; justify-content:flex-end; padding:20px; pointer-events:none; border-radius: var(--r-8); }
...
.studio-v3-lb-hover-tag { font-size:11px; color:rgba(255,255,255,0.85); background:rgba(255,255,255,0.15); padding:3px 9px; border-radius: var(--r-20); backdrop-filter:blur(4px); }
.studio-v3 .portfolio-match-dot { position:absolute; top:7px; right:7px; width:7px; height:7px; border-radius:50%; background:var(--sage); box-shadow:0 0 0 2px rgba(255,255,255,0.7); }
.studio-v3 .portfolio-group-divider { grid-column:1 / -1; display:flex; align-items:center; gap:10px; padding:10px 0 4px; }
.studio-v3 .portfolio-group-label { font-size:10px; letter-spacing:0.14em; text-transform:uppercase; color:var(--taupe); white-space:nowrap; }
.studio-v3 .portfolio-group-divider::after { content:''; flex:1; height:1px; background:var(--surface3); }
.studio-v3 .portfolio-see-more { margin-top:14px; display:flex; align-items:center; gap:8px; cursor:pointer; background:none; border:none; font-family:'DM Sans',sans-serif; font-size:12px; color:var(--text-muted); letter-spacing:0.06em; padding:0; transition:color 0.15s; width:100%; justify-content:center; }
.studio-v3 .portfolio-see-more:hover { color:var(--sage); }
.studio-v3 .portfolio-see-more svg { transition:transform 0.2s; }
.studio-v3 .portfolio-see-more.open svg { transform:rotate(180deg); }

/* ── USPs ── */
.studio-v3 .usp-box { background:var(--surface); border:1px solid #F0D4A4; border-radius: var(--r-10); padding:4px 0; }
...
.studio-v3 .h-tab { padding:10px 18px; cursor:pointer; font-size:11px; letter-spacing:0.1em; text-transform:uppercase; color:var(--text-muted); border:none; border-bottom:2px solid transparent; background:none; font-family:'DM Sans',sans-serif; white-space:nowrap; transition:all 0.15s; flex-shrink:0; }
.studio-v3 .h-tab:hover { color:var(--text-heading); }
.studio-v3 .h-tab.active { color:var(--text-heading); border-bottom-color:var(--sage); }
...
.studio-v3 .craft-detail-stats { display:flex; gap:24px; border-top:1px solid var(--surface2); padding-top:14px; flex-wrap:wrap; }
.studio-v3 .craft-stat-label { font-size:10px; letter-spacing:0.12em; text-transform:uppercase; color:var(--text-muted); margin-bottom:3px; }
.studio-v3 .craft-stat-val { font-size:14px; font-weight:500; color:var(--text-heading); }
.studio-v3 .craft-list { overflow-y:auto; max-height:360px; }
.studio-v3 .craft-list-item { display:flex; flex-direction:column; gap:4px; width:100%; text-align:left; padding:13px 16px; border:none; border-bottom:1px solid var(--surface2); background:none; font-family:'DM Sans',sans-serif; cursor:pointer; transition:background 0.12s; }
.studio-v3 .craft-list-item:last-child { border-bottom:none; }
.studio-v3 .craft-list-item:hover { background:var(--bg); }
.studio-v3 .craft-list-item.active { background:var(--surface); }
.studio-v3 .craft-list-name { font-size:13px; font-weight:500; color:var(--text-heading); }
.studio-v3 .craft-empty { font-size:13px; color:var(--taupe); font-style:italic; padding:20px 16px; }

/* ── FABRICS & DYES ── */
.studio-v3 .fabric-tags { display:flex; flex-wrap:wrap; gap:8px; }
.studio-v3 .fabric-tag { display:inline-flex; align-items:center; gap:7px; font-size:13px; padding:6px 13px; background:#fff; border:1px solid var(--surface4); border-radius:24px; color:var(--text-heading); }
.studio-v3 .dye-tag { display:inline-flex; align-items:center; gap:7px; font-size:13px; padding:6px 13px; background:var(--surface); border:1px solid var(--border-l); border-radius:24px; color:#5A4E40; }

/* ── CATEGORIES ── */
.studio-v3 .cat-rows { display:flex; flex-direction:column; gap:0; border-left:2px solid var(--surface3); padding-left:16px; }
.studio-v3 .cat-row { display:flex; flex-wrap:wrap; align-items:baseline; gap:0; margin-bottom:0; line-height:1; }
.studio-v3 .cat-row-label { font-size:10px; letter-spacing:0.15em; text-transform:uppercase; color:var(--taupe); margin-right:12px; white-space:nowrap; flex-shrink:0; padding:5px 0; }
.studio-v3 .cat-kw { font-size:13px; color:var(--text-dark); padding:4px 0; margin-right:0; }
.studio-v3 .cat-kw::after { content:' · '; color:var(--warm-gray); margin:0 6px; }
...
.studio-v3 .collab-card { background:var(--surface); border:1.5px solid var(--sage); border-radius: var(--r-10); padding:18px; }
.studio-v3 .collab-icon { margin-bottom:10px; line-height:1; display:flex; }
.studio-v3 .collab-dot { display:inline-block; width:6px; height:6px; border-radius:50%; background:var(--sage); margin-right:5px; vertical-align:middle; }
.studio-v3 .collab-title { font-size:13px; font-weight:500; margin-bottom:4px; color:var(--text-heading); }
.studio-v3 .collab-desc { font-size:12px; color:var(--text-muted); line-height:1.45; }
.studio-v3 .design-caps { display:flex; flex-wrap:wrap; gap:8px; }
.studio-v3 .design-cap { display:inline-flex; align-items:center; gap:7px; font-size:13px; padding:7px 14px; background:#fff; border:1px solid var(--surface4); border-radius: var(--r); color:var(--text-dark); }

/* ── INSIDE THE STUDIO ── */
.studio-v3 .bts-strip { display:flex; gap:10px; overflow-x:auto; padding-bottom:6px; scrollbar-width:thin; scrollbar-color:var(--warm-gray) transparent; }
.studio-v3 .bts-thumb { flex-shrink:0; width:170px; height:125px; border-radius: var(--r-8); overflow:hidden; position:relative; background:var(--surface3); cursor:pointer; transition:transform 0.15s; }
...
.studio-v3 .bts-play span { width:26px; height:26px; border-radius:50%; background:rgba(255,255,255,0.8); display:flex; align-items:center; justify-content:center; font-size:10px; color:var(--text); padding-left:2px; }
...
.studio-v3 .recog-item { display:flex; align-items:center; gap:12px; font-size:13.5px; padding:11px 14px; background:#fff; border:1px solid var(--surface4); border-radius: var(--r-8); color:var(--text-dark); }
.studio-v3 .recog-item a { color:var(--text-dark); text-decoration:underline; text-decoration-color:rgba(181,147,90,0.5); }
...
.studio-v3 .cert-tag { display:inline-flex; align-items:center; gap:5px; font-size:11px; letter-spacing:0.06em; text-transform:uppercase; padding:4px 10px; background:var(--surface2); border:1px solid #BCCFB8; border-radius: var(--r-20); color:var(--green-d); font-weight:500; }

/* ── TEAM ── */
.studio-v3 .team-card { display:flex; gap:18px; background:#fff; border:1px solid var(--surface4); border-radius: var(--r-10); padding:20px; }
...
.studio-v3 .team-role { font-size:12px; color:var(--text-muted); margin-bottom:10px; }
.studio-v3 .team-contact-badge { display:inline-block; font-size:10px; letter-spacing:0.08em; text-transform:uppercase; background:var(--surface2); color:var(--green-d); border-radius: var(--r-4); padding:2px 7px; margin-left:6px; }
.studio-v3 .team-bio { font-size:13px; line-height:1.6; color:#5A4E40; white-space:pre-line; }

/* ── SIDEBAR STATS BLOCK ── */
.studio-v3 .s-stats-block { display:flex; align-items:center; gap:0; margin-bottom:14px; background:var(--bg); border:1px solid var(--surface3); border-radius: var(--r-8); overflow:hidden; }
.studio-v3 .s-stat { flex:1; padding:12px 14px; text-align:center; }
.studio-v3 .s-stat-num { font-family:'Cormorant Garamond',Georgia,serif; font-size:28px; font-weight:500; color:var(--text-heading); line-height:1; margin-bottom:4px; }
.studio-v3 .s-stat-label { font-size:10px; letter-spacing:0.1em; text-transform:uppercase; color:var(--text-muted); }
.studio-v3 .s-stats-divider { width:1px; background:var(--surface3); align-self:stretch; flex-shrink:0; }

/* ── SIDEBAR ── */
.studio-v3 .s-card { background:#fff; border:1px solid var(--surface4); border-radius: var(--r-lg); padding:20px; margin-bottom:14px; }
.studio-v3 .s-cta-studio { font-family:'Cormorant Garamond',Georgia,serif; font-size:20px; font-weight:500; margin-bottom:5px; }
.studio-v3 .s-cta-sub { font-size:13px; color:var(--text-muted); line-height:1.5; margin-bottom:16px; }
.studio-v3 .btn-cta { display:block; width:100%; padding:12px; background:var(--sage); color:#fff; text-align:center; border:none; border-radius: var(--r-8); font-size:13.5px; font-family:'DM Sans',sans-serif; cursor:pointer; letter-spacing:0.04em; transition:background 0.2s; }
.studio-v3 .btn-cta:hover { background:var(--sage-d); }
.studio-v3 .s-glance-label { font-size:10px; letter-spacing:0.18em; text-transform:uppercase; color:var(--text-muted); margin-bottom:14px; }
.studio-v3 .s-row { display:flex; justify-content:space-between; align-items:baseline; padding:9px 0; border-bottom:1px solid var(--surface2); font-size:13px; }
.studio-v3 .s-row:last-child { border-bottom:none; }
.studio-v3 .s-key { color:var(--text-muted); }
.studio-v3 .s-val { color:var(--text-heading); font-weight:500; text-align:right; max-width:55%; line-height:1.35; }
.studio-v3 .s-row-expandable { cursor:pointer; width:100%; background:none; border:none; border-bottom:1px solid var(--surface2); font-family:'DM Sans',sans-serif; }
.studio-v3 .s-row-expandable:hover .s-key { color:var(--sage); }
.studio-v3 .s-moq-detail { background:var(--bg); border-radius:0 0 8px 8px; margin:-2px 0 6px; padding:10px 12px; border:1px solid var(--surface2); border-top:none; }
.studio-v3 .s-moq-item { font-size:12px; color:#6B5F4A; padding:3px 0; }
.studio-v3 .s-moq-note { font-size:11px; color:var(--text-muted); margin-top:8px; font-style:italic; }
.studio-v3 .s-pricing { display:flex; gap:1px; align-items:center; justify-content:flex-end; }
.studio-v3 .s-pricing-symbol { font-size:14px; font-weight:600; color:var(--sage); }
.studio-v3 .s-pricing-symbol.dim { color:var(--warm-gray); font-weight:400; }
...
.studio-v3-modal-select { width:100%; border:1px solid var(--border-l); border-radius: var(--r); padding:10px 12px; font-size:13px; font-family:'DM Sans',sans-serif; color:var(--text-heading); background:#fff; outline:none; margin-bottom:22px; appearance:none; background-image:url("data:image/svg+xml,%3Csvg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='%239A8F82' stroke-width='2' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M6 9l6 6 6-6'/%3E%3C/svg%3E"); background-repeat:no-repeat; background-position:right 12px center; padding-right:36px; }
.studio-v3-modal-select:focus { border-color:var(--sage); }
.studio-v3-modal-input { width:100%; border:1px solid var(--border-l); border-radius: var(--r); padding:10px 12px; font-size:13px; font-family:'DM Sans',sans-serif; color:var(--text-heading); background:#fff; outline:none; margin-bottom:16px; box-sizing:border-box; }
.studio-v3-modal-input:focus { border-color:var(--sage); }
.studio-v3-modal-actions { display:flex; gap:10px; flex-direction:column; }
.studio-v3-btn-primary { padding:12px; background:var(--sage); color:#fff; border:none; border-radius: var(--r-8); font-size:13.5px; font-family:'DM Sans',sans-serif; cursor:pointer; transition:background 0.2s; display:flex; align-items:center; justify-content:center; gap:8px; }
.studio-v3-btn-primary:hover:not(:disabled) { background:var(--sage-d); }
.studio-v3-btn-primary:disabled { background:var(--border-l); cursor:not-allowed; }
.studio-v3-btn-secondary { padding:11px; background:#fff; color:#5A4E40; border:1px solid var(--border-l); border-radius: var(--r-8); font-size:13px; font-family:'DM Sans',sans-serif; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px; }
.studio-v3-btn-secondary:hover:not(:disabled) { border-color:var(--text-muted); color:var(--text-heading); }
.studio-v3-modal-close { position:absolute; top:16px; right:18px; background:none; border:none; font-size:20px; color:var(--text-muted); cursor:pointer; line-height:1; }
.studio-v3-modal-err { font-size:12px; color:#B4453C; margin-bottom:14px; padding:8px 12px; background:var(--surface); border-radius: var(--r); border:1px solid rgba(180,69,60,0.2); }
...
.studio-v3-spin.dark { border-color:rgba(0,0,0,0.15); border-top-color:var(--text-heading); }
...
@media (max-width: 700px) {
  ...
  .studio-v3 .craft-detail { border-right:none; border-bottom:1px solid var(--surface4); }
...
const IconGrid = () => (
  <svg width="22" height="22" viewBox="0 0 22 22" fill="none" stroke="var(--sage)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} className="studio-v3-lb-frame"
        ...
        {isVideo ? (
          <video ref={videoRef} key={lbSrc} src={mediaUrl(lbSrc)} autoPlay loop muted={muted} playsInline
            style={{ maxWidth: '88vw', maxHeight: '80vh', borderRadius: 'var(--r-8)', display: 'block', outline: 'none' }} />
        ) : (
          <img src={mediaUrl(lbSrc)} alt=""
            style={{ maxWidth: '88vw', maxHeight: '80vh', width: 'auto', height: 'auto', objectFit: 'contain', display: 'block', borderRadius: 'var(--r-8)' }} />
        )}
      ...
      <div style={{ position: 'absolute', bottom: 20, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 8, maxWidth: '90vw', overflowX: 'auto', padding: '4px 0' }}>
        {images.map((img, i) => {
          ...
          const frame = {
            width: 52, height: 40, borderRadius: 'var(--r-5)', cursor: 'pointer', flexShrink: 0,
            border: i === idx ? '2px solid var(--sage)' : '2px solid transparent',
      ...
      <div className="studio-v3-modal-box" onClick={e => e.stopPropagation()}>
        ...
        {done ? (
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 32, marginBottom: 12, color: 'var(--sage)' }}>✓</div>
...
export default function StudioProfile() {
  ...
  if (loading) return (
    <div className="studio-v3">
      ...
      <div style={{ height: 50, background: '#fff', borderBottom: '1px solid var(--surface3)' }} />
      <div style={{ height: '48vh', minHeight: 320, background: 'var(--surface2)' }} />
                ...
                <>
                  <button className="s-row s-row-expandable" onClick={() => setMoqExpanded(x => !x)}>
                    ...
                    <span className="s-val">
                      {minimumsLabel}
                      <span style={{ fontSize: 10, color: 'var(--text-muted)', marginLeft: 5, display: 'inline-block', transition: 'transform 0.2s', transform: moqExpanded ? 'rotate(180deg)' : 'none' }}>▾</span>
                    </span>
                  </button>
                  {moqExpanded && (
                      ...
                      {prod.moq_entries?.length > 0
                        ? prod.moq_entries.map((m, i) => (
                            <div className="s-moq-item" key={i}>
                              <span style={{ fontWeight: 500, color: 'var(--text2)' }}>{m.craft_or_category}</span>
