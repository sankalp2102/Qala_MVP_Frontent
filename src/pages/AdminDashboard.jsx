...
function TradeShowEnquiryDetail({ enquiryId, onBack }) {
  ...
  async function handleSubmitOrder() {
    ...
    setSubmitting(true);
    try {
      // Bug fix (Sep 2026): payment_link moved OUT of Submit and onto
      // the schedule screen instead, alongside deposit_amount — see
      // handleSchedule below. A payment link is normally generated FOR
      // a specific amount, so asking for it here, before that amount
      // even exists on screen, had the two backwards.
      await adminAPI.submitTradeShowEnquiry(enquiryId, { delivery_date: deliveryDate });
  ...
  async function handleSchedule() {
    ...
    // Bug fix (Sep 2026): payment link moved here from Submit, for the
    // same reason as deposit amount above — same type-agnostic-backend
    // pattern, same "send '' for non-Order" approach.
    if (enquiry.enquiry_type === 'order' && !paymentLink.trim()) {
      error('Payment link is required to schedule an order');
      return;
    }
    ...
    try {
      const body = {
        ...
        payment_link: enquiry.enquiry_type === 'order' ? paymentLink : '',
      };
    ...
    <div style={{ maxWidth: 980, margin: '0 auto', padding: '32px 24px' }}>
      ...
      {effectiveStep === 3 && (
        <div style={{ border: '1px solid var(--surface4)', borderRadius: 'var(--r)', background: '#fff', padding: 20 }}>
          ...
          {/* Bug fix (Sep 2026): payment link moved to the schedule
              screen, alongside deposit amount — see that section below.
              Delivery date is the only thing this step still collects. */}
          <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <button
              ...
              style={{ ...IR_BTN_BASE, background: '#1A1A1A', color: '#fff', opacity: (submitting || !deliveryDate) ? 0.6 : 1 }}
            >
      ...
      {isSubmitted && (
        <div style={{ border: '1px solid var(--surface4)', borderRadius: 'var(--r)', background: '#fff', padding: 20 }}>
          ...
          {/* Feature (Sep 2026): its own visually separate section,
              ...
              decision (how much to ask for, and where payment goes) from
              timing (when to send), and the boxed styling here makes
              that visible rather than just implied by field order.
              Order type only — Enquiry never mentions either field in
              its email at all.
              Bug fix (Sep 2026): payment link moved here from Submit,
              at explicit request — a payment link is normally generated
              FOR a specific amount, so it only makes sense to ask for it
              once that amount is actually visible, not before. Deposit
              amount sits first in this same box for exactly that
              reason. */}
          {enquiry.enquiry_type === 'order' && (
            <div style={{ border: '1px solid var(--surface4)', borderRadius: 'var(--r-5)', background: 'var(--surface2)', padding: 16, marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Payment link *</div>
              <input
                type="url"
                value={paymentLink}
              ...
              <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 6 }}>
                Generate this for the deposit amount above, then paste it here.
              </div>
            </div>
          )}
