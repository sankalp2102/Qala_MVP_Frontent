...
function DispatchModal({ order, projectId, onClose, onDone }) {
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:8000, background:'rgba(26,14,8,0.6)', backdropFilter:'blur(6px)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div onClick={e => e.stopPropagation()} style={{ background:'var(--surface)', borderRadius:16, padding:'28px 32px', width:'100%', maxWidth:440, border:'1px solid var(--border)' }}>
              ...
              <div key={p.id} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, overflow:'hidden' }}>
                ...
                <div style={{ padding:'20px 24px', display:'flex', justifyContent:'space-between', alignItems:'center', cursor:'pointer' }} onClick={() => toggle(p.id)}>
                  ...
                  <div style={{ display:'flex', gap:12, alignItems:'center' }}>
                    <span style={{ fontSize:11, fontWeight:600, padding:'3px 10px', borderRadius:20, background:'var(--teal-dim)', color:'var(--teal)', textTransform:'uppercase' }}>
                      {p.stage?.replace(/_/g,' ')}
                    </span>
                ...
                {isExpanded && (
                  <div style={{ borderTop:'1px solid var(--border)', padding:'20px 24px' }}>
                    ...
                    {acceptedProposal && (
                      <div style={{ background:'var(--surface2)', borderRadius:10, padding:'14px 18px', marginBottom:20, display:'flex', gap:24, flexWrap:'wrap' }}>
                    ...
                    {orders.length === 0 ? (
                      ...
                      <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                        ...
                        {orders.map(o => (
                          <div key={o.id} style={{ background:'var(--bg)', border:'1px solid var(--border)', borderRadius:10, padding:'14px 18px' }}>
                            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10, flexWrap:'wrap', gap:8 }}>
                              <div style={{ display:'flex', gap:10, alignItems:'center' }}>
                                ...
                                <span style={{ fontSize:11, fontWeight:600, color: ORDER_STATUS_COLORS[o.status] || 'var(--text3)', background:'var(--surface)', padding:'2px 8px', borderRadius:6 }}>
